...
public class CDXServer extends BaseCDXServer {
    ...
    public CDXServer() {
        cdxLineFactory = new StandardCDXLineFactory(settings.getCdxFormat());
    }

    public CDXServer(final CdxServerSettings settings) {
        super(settings);
        cdxLineFactory = new StandardCDXLineFactory(settings.getCdxFormat());
    }
...
//			}
            ...
            if (query.page >= 0 || query.showNumPages) {
                ...
                maxLimit = settings.getQueryMaxLimit();
            }
    ...
    protected CloseableIterator<String> createPagedCdxIterator(
            ...
            CDXWriter responseWriter) throws IOException {
        if (!(settings.getCdxSource() instanceof ZipNumCluster)) {
            responseWriter
                    .printError("Sorry, this server is not configured to support paged query. Remove page= param and try again.");
            return null;
        }

        ZipNumCluster zipnumSource = (ZipNumCluster) settings.getCdxSource();

        boolean allAccess = getAuthChecker().isAllUrlAccessAllowed(authToken);

        if ((query.pageSize <= 0)
                || ((query.pageSize > settings.getMaxPageSize()) && !allAccess)) {
            query.pageSize = settings.getMaxPageSize();
        }
    ...
    protected CloseableIterator<String> createBoundedCdxIterator(
        ...
        ZipNumParams params = new ZipNumParams(settings.getDefaultZipNumParams());
...
//            }
        ...
        if (pageResult != null) {
            ...
            return ((ZipNumCluster) settings.getCdxSource())
                    .getCDXIterator(idx, searchKey, startEndUrl[1],
                            query.page, pageResult.numPages, params);
        } else {
            return settings.getCdxSource().getCDXIterator(searchKey, startEndUrl[0],
    ...
    protected void writeCdxResponse(CDXWriter responseWriter,
        ...
        if (query.collapseTime > 0) {
            if (settings.isCollapseToLast()) {
                outputProcessor = new DupeTimestampLastBestStatusFilter(
                        outputProcessor, query.collapseTime, settings.getNoCollapsePrefix());
            } else {
        ...
        if (!getAuthChecker().isAllCdxFieldAccessAllowed(authToken)) {
            outputFields = this.getAuthChecker().getPublicCdxFormat();
        }
...
//
    ...
    /**
     * Look up the latest (non-revisit) capture of {@code url} in the CDX database. If
     * {@code digest} is non-{@code null}, return only a capture with identical digest.
     * <p>
     * @param url URL (in regular form) to look for
     * @param digest content digest in the same format as CDX database, or {@code null} if any
     * version qualifies.
