...
public class DisableJSBlocksRewriteRule extends RewriteRule {
	...
	private static final Logger LOGGER = Logger.getLogger(
			DisableJSBlocksRewriteRule.class.getName());
	...
	public String rewrite(ReplayParseContext context, String policy, String input) {
		...
		if (index >= 0) {
			StringTokenizer tokens = new StringTokenizer(policy.substring(index + 1), " ]");
		...
		if (rewrite) {
			LOGGER.info(policy + ": disable script block " + scriptIndexStr + ", replacing it with \"" + removeMsg + "\"");		
			input = removeMsg;
		}
